...
const Home: NextPage = () => {
  ...
  const [parsedConfig, setParsedConfig] = useState<
    false | { format: string; settings: string }
  >(false);
  ...
  useEffect(() => {
    ...
    const newConfig = ChatChatEssentialsChatConverter.convert({
      config,
  ...
  return (
    <div>
      <Head>
        ...
        <meta
          name="description"
          content="Convert EssentialsChat Configs to ChatChat"
        />
      </Head>
      ...
      >
        ...
        <div
          css={tw`flex flex-col md:flex-row flex-grow flex-shrink h-full max-w-full`}
        >
          <div
            css={css`
          ...
          >
            ...
            <div css={tw`md:h-1/2`}>
              <TextBox
                title={"EssentialsChat Language File"}
                code={lang}
                editor={setLang}
                language={"properties"}
              />
          ...
          >
            {error || !parsedConfig ? (
              ...
              <>
                <div
                  css={tw`flex flex-col flex-grow flex-shrink h-full max-w-full`}
                >
                  <div css={tw`h-1/2`}>
                    <TextBox
                      title={"ChatChat formats.yml"}
