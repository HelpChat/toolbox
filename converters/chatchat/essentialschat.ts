...
const ChatChatEssentialsChatConverter = new Converter<EssentialsChatTypes,
    ...
    Convert(essentialschatConfig) {
        ...
        if (essentialschatConfig.essentials.chat.format) {
            ...
            ccFormat.parts.format = [Placeholders(parsed)];
        ...
        if (essentialschatConfig.language.msgFormat) {
            chatchatSettingsConfig["sender-format"].parts.format =
                [MiniMessage(essentialschatConfig.language.msgFormat)
                ];
        }

        if (essentialschatConfig.language.socialSpyMsgFormat) {
            chatchatSettingsConfig["social-spy-format"].parts.format = [
